...
\pgfdeclaremetadecoration{middle}{initial}{
    \state{initial}[
        width={(\the\pgfdecorationsegmentlength*1.075pt+3pt)},
        next state=middle
    ]
    {\decoration{moveto}}
...
\pgfdeclaremetadecoration{midd}{initial}{
    \state{initial}[
        width={(\the\pgfdecorationsegmentlength*1.075pt+6.5pt)},
        next state=middle
    ]
    {\decoration{moveto}}
...
\tikzset{fix borders/.style={decoration={middle},decorate, segment length=#1}}
\tikzset{fix corner/.style={decoration={midd},decorate, segment length=#1}}
                ...
                \ifthenelse{\diff=0}{
                    ...
                    \ifthenelse{\db=#1}{
                        ...
                        \foreach\i[count=\j from 0] in {0,...,\tmo}{
                            ...
                            \draw[thick,fix borders=#1] (1\i)--(2\i);
                            \draw[thick,fix borders=#1] (2\i)--(1\i);
                    ...
                    % No edges. Just draw the sets in a compact way
                        ...
                        \ifthenelse{#2=1}{
                            ...
                            \foreach \i in {#1,...,1} { \foreach \j in {#1,...,1} {
                                    \ifthenelse{\i=\j}{}{
                                        \draw[thick,fix corner=#1](p#1.corner \i)--(p#1.corner \j);
                                        \draw[thick,fix corner=#1](p#1.corner \j)--(p#1.corner \i);
                                    }
                            }}
                        }{
                        \ifthenelse{#2=2}{
                            ...
                            \foreach\k in {1,...,\ecnt}{
                                ...
                                \pgfmathsetmacro{\cplib}{{\poli}[\j-1])}
                                \pgfmathsetmacro{\cplcb}{{\polc}[\j-1])}
                                \draw[thick,fix corner=#1](p\cplia.corner \cplca)--(p\cplib.corner \cplcb);
                                \draw[thick,fix corner=#1](p\cplib.corner \cplcb)--(p\cplia.corner \cplca);
                            }
                            ...
                            \foreach\i in {1,...,\cnt}{
                                \pgfmathsetmacro{\curx}{{\prsx}[\i-1]}
                                \pgfmathsetmacro{\cury}{{\prsy}[\i-1]}
                                \draw[thick,fix corner=#1](o#1.corner \curx)--(o#1.corner \cury);
                                \draw[thick,fix corner=#1](o#1.corner \cury)--(o#1.corner \curx);
                            }}{}
