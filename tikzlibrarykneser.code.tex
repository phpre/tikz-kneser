...
\tikzset{kvertex/.style={circle, fill, inner sep=1.5pt, outer sep=3pt}}
...
\def\ldraw(#1)--(#2);{
    \ifx\tikz@fillcolor\pgfutil@empty
        \draw[thick] (#1) -- (#2);
    \else
        \draw[double=black,draw=\tikz@fillcolor] (#1) -- (#2);
    \fi
}
\def\bdraw(#1)--(#2);{
    \ifx\tikz@fillcolor\pgfutil@empty
    ...
    \else
        \draw[double=black,draw=\tikz@fillcolor] (#1) to[bend left] (#2);
    \fi
}
...
\def\lnode(#1);{
    \ifx\tikz@fillcolor\pgfutil@empty
        \node[kvertex,\@vfc,draw=white,line width=1pt] at (#1) {};
    \else
        \node[kvertex,\@vfc,draw=\tikz@fillcolor,line width=1pt] at (#1) {};
    \fi
}
                            ...
                            \ifthenelse{\nodd=1}{
                                ...
                                \foreach\lid in {1,...,\nly}{
                                    ...
                                    \foreach \j in {\bnd,...,\bend} {
                                        \ifthenelse{#1=6}{
                                            \ifthenelse{\lid=1}{
                                                \pgfmathsetmacro{\idx}{int(mod(\j+2, \curn)+1)}
                                            }{
