\ProvidesFileRCS{tikzlibrarykneser.code.tex}
...
\newcommand\numnext[3]{%
    ...
    \foreach \@i in {1,...,#2}{%
        ...
        \ifthenelse{\@done=1}{
            \ifthenelse{\@cc=1}{
                \pgfmathsetmacro{\@td@}{int(\@result+(\@cc*\@car))}
            ...
            \ifthenelse{\@cc=1}{
                ...
                \ifthenelse{\@cnt>0}{
                    \global\def\@done{1}
                    \pgfmathsetmacro{\@t@}{int(\@result+\@car)}
                    \global\let\@result=\@t@
...
\tikzset{
    ...
    % #1 n
    % #2 k
    % Takes lorg if n > 7; dies if n > 14
                ...
                \ifthenelse{\diff=0}{
                    ...
                    \ifthenelse{\db=#1}{
                        ...
                        \foreach\i[count=\j from 0] in {0,...,\tmo}{
                            ...
                            \pgfmathsetmacro{\nx}{int(-#1*2-12 + (-div(\tmo,\rem)/2+div(\i,\rem))*(#1*2+12+50+#1*2))}
                            \pgfmathsetmacro{\nxx}{int(#1*2+12 + (-div(\tmo,\rem)/2+div(\i,\rem))*(#1*2+12+50+#1*2))}
                    ...
                    % No edges. Just draw the sets in a compact way
                        \pgfmathsetmacro{\tmo}{int(factorial(#1)/(factorial(#1-#2)
                        *factorial(#2)))}
                        \def\strt{0}
                        \foreach\d in {1,...,#2}{
                            \pgfmathsetmacro{\ttt}{int(2*\strt+1)}
